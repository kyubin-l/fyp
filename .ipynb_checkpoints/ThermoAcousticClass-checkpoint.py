...
def det(w, Ru, Rd):
    """
    Function to calculate the determinent of the matrix from the matrix equation.
    w: frequency to be solved for
    Ru: Upstream reflection coefficient
    Rd: Downstream reflection coefficient
    """
...
def det_zero(Ru, Rd):
    """
    Returns the values of w, which make the determinent of the matrix in the matrix method to 0.
    """
    w = sp.Symbol('w')
    wl = sp.solveset(det(w, Ru, Rd), w, domain=sp.S.Complexes)
...
class ThermoacousticModel:
    ...
    def __init__(self, c, L, Ru, Rd, damping=True, temperature_gradient=True):
        ...
        x = np.linspace(0, self.L, 1000)[..., np.newaxis]
